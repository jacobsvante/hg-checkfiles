...
""" Detects (and optionally fixes) tabs and trailing whitespace in committed
    files

== The hooks ==

check_hook: prevents commits containing tabs or trailing whitespace, without
            actually touching any files. Use as pretxncommit locally or
            pretxnchangegroup on a central repo.

fixup_hook: automatically fixes any problematic files before commit. Useful in
            pre-commit. (As this hook may change files on disc, you will need
            to recompile your project after committing if any files needed
            fixing.)

             If you're not comfortable with this kind of magic, use the
             check_hook and manually run the command with the --fixup option.
...
checks changed files in the working directory for tabs or trailing whitespace
    ...
    - --debug shows settings and details about each file considered for
              checking
...
options:
 ...
 -t --tabsize VALUE  set the tab length (default: 4)
    --all            fix all files in working directory, not just the changes
                     ones
...
checked_exts = .c .h .cpp .xml .cs .html .js .css .txt .py .nsi .java

# default is .sln .suo .vcproj .csproj .ui
ignored_exts = .xslt
ignored_files = foo/contains_tabs.txt bar/contains_trailing_ws.txt
# ignore everything in the /res directory, including those files in
# subdirectories.
ignored_patterns = res/.*
tab_size = 4

# For check_hook to only check for tabs (and ignore trailing whitespace) set
...
# to replace spaces with tabs (instead of tabs -> spaces), set use_spaces
# to False.
use_spaces = True
...
class CheckFiles(object):
    ...
    def __init__(self, ui, repo, ctx, opts = {}):
        ...
        self.ignored_exts = ui.configlist('checkfiles', 'ignored_exts',
            # some common autogenerated filetypes
            default='.sln .suo .vcproj .csproj .ui')
        ...
        self.check_ignores_trailing_ws = ui.configbool('checkfiles',
                                            'check_ignores_trailing_ws', False)
    ...
    def set_changectx(self, ctx):
        ...
        if self.opt_all:
            modified, added, removed, deleted, unknown, ignored, clean = \
                                               self.repo.status(clean=True)
            # We can't get filecontext for unknown files
            self.files = modified + added + clean
        else:
    ...
    def is_relevant(self, file):
        ...
        if any(map(lambda e: re.search(e, file) is not None,
                   self.ignored_patterns)):
...
def fixup_hook(ui, repo, hooktype, **kwargs):
    """ Removes tabs and/or trailing whitespace from modified files in the
        working directory
    """

    ui.note('checkfiles: removing tabs and/or trailing whitespace in ' \
            'changed files...\n')
...
def check_cmd(ui, repo, **opts):
    """ Checks changed files in the working directory for tabs or trailing
        whitespace
    ...
    ui.note('checkfiles: checking %s files for tabs or trailing ' \
...
def fixup_cmd(ui, repo, **opts):
    """ Replaces tabs with spaces and removes trailing whitespace from changed
        files
    """

    ui.note('checkfiles: removing tabs and/or trailing whitespace in %s ' \
            'files...\n' % ('all' if opts['all'] else 'modified'))
...
cmdtable = {
    'checkfiles': (check_cmd,
                   [('t', 'tabsize', 4, 'set the tab length'),
                     ('', 'all', None, 'check all tracked files (not just ' \
                                       'changed)'),
                     ('', 'diff', None, 'check only diff lines (not entire ' \
                                        'file)')],
                   'hg checkfiles [options]'),

    'fixwhitespace': (fixup_cmd,
                     [('t', 'tabsize', 4, 'set the tab length'),
                      ('', 'all', None, 'fix all tracked files (not just ' \
                                        'changed)'),
                      ('', 'diff', None, 'fix only diff lines (not entire ' \
                                         'file)')],
                      'hg fixwhitespace [options]'),
}
